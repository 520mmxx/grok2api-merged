...
import { getProxyPool, resetProxyPoolInstance } from "./proxy/pool";

const app = new Hono<{ Bindings: Env }>();

// Lazy-init proxy pool from env vars on first request
let proxyPoolInitialized = false;
async function bootstrapProxyPool(env: Env): Promise<void> {
  if (proxyPoolInitialized) return;
  ...
  try {
    ...
    console.error("[proxy-pool] Bootstrap failed:", e);
    resetProxyPoolInstance();
  }
}
...
// Bootstrap proxy pool on first request
app.use("*", async (c, next) => {
  await bootstrapProxyPool(c.env as Env);
  return next();
});
...
app.get("/health", async (c) => {
  ...
  return c.json({
    ...
    bindings: {
      ...
      assets: Boolean(getAssets(env)),
    },
    proxy_pool: proxyPoolInfo,
  });
});

  ...
  try {
    ...
    return c.json({
      ...
      grok: {
        ...
        proxy_urls: String(settings.grok.proxy_urls ?? ""),
        proxy_pool_url: String(settings.grok.proxy_pool_url ?? ""),
        proxy_pool_interval: Number(settings.grok.proxy_pool_interval ?? 300),
...
adminRoutes.post("/api/v1/admin/config", requireAdminAuth, async (c) => {
  try {
    ...
    if (grokCfg && typeof grokCfg === "object") {
      ...
      if (typeof grokCfg.proxy_urls === "string") grok_config.proxy_urls = grokCfg.proxy_urls.trim();
      if (typeof grokCfg.proxy_pool_url === "string") grok_config.proxy_pool_url = grokCfg.proxy_pool_url.trim();
      if (Number.isFinite(Number(grokCfg.proxy_pool_interval)))
        grok_config.proxy_pool_interval = Math.max(10, Math.floor(Number(grokCfg.proxy_pool_interval)));
